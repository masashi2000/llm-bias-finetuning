...
class Agent:
    def __init__(self, name, persona, party):
        ...
        self.party = party
        self.conversation_history = ""  # エージェントごとの会話履歴を保持するために追加

    def generate_response(self, prompt, generator):
        """エージェントの会話応答を生成"""
...
class Session:
    ...
    def initialize_agents(self):
        for agent in self.agents:
            agent.conversation_history = ""  # 各エージェントの履歴を初期化
    ...
    def update_survey_results(self, responses, round_number):
        ...
        for agent, response in zip(self.agents, responses):
            ...
            survey_response = int(numbers[0]) if numbers else -1
...
class Experiment:
    ...
    def run(self, generator, output_dir):
        ...
        # バッチ処理するために一度すべてのセッションを生成する。
        print("\nNow generating all sessions for batch processings\n")
